    ...
    match std::fs::read_to_string(path) {
      ...
      Ok(content) => match serde_json::from_str::<Self>(&content) {
        ...
        Err(e) => {
          ...
          let error = VestaboardError::json_error(e, &format!("parsing playlist from {}", path.display()));
          if !silent {
      ...
      Err(e) => {
        ...
        let error = VestaboardError::io_error(e, &format!("reading playlist from {}", path.display()));
        if !silent {
          print_error(&error.to_user_message());
        }
        Err(error)
  ...
  fn save_internal(&self, path: &Path, silent: bool) -> Result<(), VestaboardError> {
    ...
    if let Some(parent) = path.parent() {
      if let Err(e) = std::fs::create_dir_all(parent) {
        let error = VestaboardError::io_error(e, &format!("creating directory for {}", path.display()));
        if !silent {
          print_error(&error.to_user_message());
        }
        return Err(error);
    ...
    let content =
      serde_json::to_string_pretty(self).map_err(|e| VestaboardError::json_error(e, "serializing playlist"))?;
    ...
    match std::fs::write(path, content) {
      ...
      Err(e) => {
        ...
        let error = VestaboardError::io_error(e, &format!("saving playlist to {}", path.display()));
        if !silent {
          print_error(&error.to_user_message());
        }
        Err(error)
...
pub fn remove_item_from_playlist(id: &str) -> Result<(), VestaboardError> {
  ...
  if !playlist.remove_item(id) {
    return Err(VestaboardError::validation_error(&format!("Item '{}' not found in playlist", id)));
  }
