...
mod tests {
    ...
    use crate::{log_widget_start, log_widget_success, log_widget_error, 
...
console_log_level = "info""#,
        ...
        let config_path = std::path::PathBuf::from("data/vblconfig.toml");
        ...
        assert!(last_line.contains("Test log entry for timestamp validation"), 
        ...
        let timestamp_pattern = regex::Regex::new(r"^\d{4}-\d{2}-\d{2} \d{2}:\d{2}:\d{2}\.\d{3} \[INFO\]")
            .expect("Regex should compile");
        
        assert!(timestamp_pattern.is_match(last_line), 
